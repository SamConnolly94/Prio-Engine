...
bool CGameText::Initialise(ID3D11Device * device, ID3D11DeviceContext * deviceContext, HWND hWnd, int screenWidth, int screenHeight, D3DXMATRIX baseViewMatrix)
{
	...
	if (!mpFont)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to the font pointer in CGameText.");
		return false;
	}
	logger->GetInstance().GetInstance().MemoryAllocWriteLine(typeid(mpFont).name());
	...
	if (!result)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to load Fonts/defaultData.txt or Fonts/font.dds or both.");
		return false;
	}
	...
	if (!result)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to initialise the font object in GameText.cpp.");
		return false;
	}
	logger->GetInstance().GetInstance().MemoryAllocWriteLine(typeid(mpFontShader).name());
...
void CGameText::Shutdown()
{
	...
	if (mpFontShader)
	{
		...
		delete mpFontShader;
		mpFontShader = nullptr;
		logger->GetInstance().GetInstance().MemoryDeallocWriteLine(typeid(mpFontShader).name());
	}
	...
	if (mpFont)
	{
		...
		delete mpFont;
		mpFont = nullptr;
		logger->GetInstance().GetInstance().MemoryDeallocWriteLine(typeid(mpFont).name());
	}
}
...
bool CGameText::Render(ID3D11DeviceContext * deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix)
{
	...
	for (auto sentence : mpSentences)
	{
		...
		if (!result)
		{
			...
			logger->GetInstance().GetInstance().WriteLine("Failed to render text.");
...
bool CGameText::InitialiseSentence(SentenceType** sentence, int maxLength, ID3D11Device * device)
{
	...
	*sentence = new SentenceType();
	if (!sentence)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to a sentence object in GameText.cpp.");
		return false;
	}
	logger->GetInstance().GetInstance().MemoryAllocWriteLine(typeid(sentence).name());
	...
	if (!vertices)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to the vertices buffer in GameText.cpp.");
		return false;
	}
	...
	if (!indices)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to the indices buffer in GameText.cpp.");
		return false;
	}
	...
	if (FAILED(result))
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to create the vertex buffer in DirectX after passing it vertex data in GameText.cpp.");
		return false;
	}
	...
	if (FAILED(result))
	{
		logger->GetInstance().GetInstance().WriteLine("Faield to create the index buffer in DirectX after passing it index data in GameText.cpp.");
		return false;
	}
...
bool CGameText::UpdateSentence(SentenceType* &sentence, std::string text, int posX, int posY, float red, float green, float blue, ID3D11DeviceContext* deviceContext)
{
	...
	{
		logger->GetInstance().GetInstance().WriteLine("Sentence was too long. Would have caused buffer overflow.");
		return false;
	}
	...
	if (!vertices)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to vertices array in GameText.cpp.");
		return false;
	}
	...
	if (FAILED(result))
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to map the resource when updating sentence in GameText.cpp.");
		return false;
	}
...
SentenceType* CGameText::CreateSentence(ID3D11Device* device, ID3D11DeviceContext* deviceContext, std::string text, int posX, int posY, int maxLength)
{
	...
	if (!result)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to initialise sentences.");
		return nullptr;
	}
	...
	if (!result)
	{
		logger->GetInstance().GetInstance().WriteLine("Failed to update sentence.");
		return nullptr;
	}
...
void CGameText::ReleaseSentence(SentenceType* sentence)
{
	if (sentence)
	{
		...
		delete (sentence);
		(sentence) = nullptr;
		logger->GetInstance().GetInstance().MemoryDeallocWriteLine(typeid(sentence).name());
	}
}
