...
bool CGraphics::Initialise(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	{
		logger->GetInstance().WriteLine("Failed to initialise the foliage shader.");
		return false;
	}
...
bool CGraphics::RenderTerrains(D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, D3DXMATRIX viewProj)
{
	...
	if (mpSceneLight)
	{
		...
		if (!mpTerrainShader->Render(mpD3D->GetDeviceContext(),
			...
			mpTerrain->GetGrassSlopeCutoff(),
			mpTerrain->GetRockSlopeCutoff()
		))
		{
			logger->GetInstance().WriteSubtitle("Critical error.");
...
bool CGraphics::RenderModels(D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, D3DXMATRIX viewProj, D3DXMATRIX reflectionViewProj)
{
	...
	if (!RenderFoliage(world, view, proj, viewProj))
		return false;
...
bool CGraphics::RenderFoliage(D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, D3DXMATRIX viewProj)
{
	if (mpTerrain == nullptr)
	{
		return true;
