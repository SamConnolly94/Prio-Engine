...
bool CGameFont::Initialise(ID3D11Device * device, char * fontDataFile, WCHAR * fontTexture)
{
	...
	if (!result)
	{
		...
		logger->GetInstance().GetInstance().WriteLine("Failed to load the font data file.");
	...
	{
		// Output a message informing the user to the log.
		logger->GetInstance().GetInstance().WriteLine("Failed to load the texture for a font.");
		// Tell whatever called this that we failed.
		return false;
	}
...
bool CGameFont::LoadFontData(char * dataFile)
{
	...
	if (!inFile.is_open())
	{
		logger->GetInstance().GetInstance().WriteLine("Could not open the input file in GameFont.cpp.");
		return false;
	}
	...
	if (!mpFont)
	{
		...
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to the mpFont array in GameFont.cpp.");
...
bool CGameFont::LoadTextureFile(ID3D11Device * device, WCHAR * fontTexture)
{
	...
	{
		// Write the failure message to the log.
		logger->GetInstance().GetInstance().WriteLine("Failed to allocate memory to mpTexture in GameFont.cpp.");
	...
	if (!result)
	{
		...
		logger->GetInstance().GetInstance().WriteLine("Failed to intialise mpTexture in GameFont.cpp.");
		// Tell whatever called this we failed.
		return false;
	}
