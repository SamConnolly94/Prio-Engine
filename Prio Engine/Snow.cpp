...
bool CSnow::Initialise(ID3D11Device* device, std::string rainTexture, unsigned int numberOfParticles)
{
	//Shutdown();
...
bool CSnow::InitialiseBuffers(ID3D11Device * device)
{
...
void CSnow::Update(float updateTime)
{
	mAge += updateTime;
}

void CSnow::UpdateRender(ID3D11DeviceContext * deviceContext)
{
	unsigned int stride;
	unsigned int offset;
...
void CSnow::Render(ID3D11DeviceContext * deviceContext)
{
	unsigned int stride;
	unsigned int offset;
...
ID3D11ShaderResourceView* CSnow::CreateRandomTexture(ID3D11Device* device)
{
...
void CSnow::SetNumberOfParticles(unsigned int numParticles)
{
	mNumberOfParticles = numParticles;
}

void CSnow::SetFirstRun(bool firstRun)
{
	mFirstRun = firstRun;
}

void CSnow::SetAge(float age)
{
	mAge = age;
}

void CSnow::SetGameTime(float gameTime)
{
	mGameTime = gameTime;
}

void CSnow::SetEmitterPos(D3DXVECTOR3 pos)
{
	mEmiterPosition = pos;
}

void CSnow::SetEmitterDir(D3DXVECTOR3 dir)
{
	mEmitterDirection = dir;
}

unsigned int CSnow::GetNumberOfParticles()
{
	return mNumberOfParticles;
}

bool CSnow::GetIsFirstRun()
{
	return mFirstRun;
}
...
float CSnow::GetGameTime()
{
	return mGameTime;
}

D3DXVECTOR3 CSnow::GetEmitterPos()
{
	return mEmiterPosition;
}

D3DXVECTOR3 CSnow::GetEmitterDir()
{
	return mEmitterDirection;
}

ID3D11ShaderResourceView * CSnow::GetRainTexture()
{
	return mpRainDropTexture->GetTexture();
}

ID3D11ShaderResourceView * CSnow::GetRandomTexture()
{
