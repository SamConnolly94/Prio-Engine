...
bool CSnowShader::Initialise(ID3D11Device * device, HWND hwnd)
{
	bool result;

	// Initialise the vertex pixel shaders.
	std::string vsFilename = "Shaders/Snow.vs.hlsl";
	std::string gsFilename = "Shaders/Snow.gs.hlsl";
	std::string psFilename = "Shaders/Snow.ps.hlsl";
	std::string vsUpdateFilename = "Shaders/SnowParticleUpdate.vs.hlsl";
	std::string gsUpdateFilename = "Shaders/SnowParticleUpdate.gs.hlsl";

	result = InitialiseShader(device, hwnd, vsFilename, gsFilename, psFilename, vsUpdateFilename, gsUpdateFilename);

	if (!result)
	{
		logger->GetInstance().WriteLine("Failed to initialise the vertex and pixel shaders when initialising the snow shader class.");
		return false;
	}
...
void CSnowShader::Shutdown()
{
	// Shutodwn the vertex and pixel shaders as well as all related objects.
	ShutdownShader();
}

bool CSnowShader::Render(ID3D11DeviceContext* deviceContext)
{
	bool result;
	...
	deviceContext->PSSetShaderResources(0, 1, &mpDiffuseTexture);
...
bool CSnowShader::UpdateRender(ID3D11DeviceContext * deviceContext)
{
	bool result;
...
bool CSnowShader::InitialiseShader(ID3D11Device * device, HWND hwnd, std::string vsFilename, std::string gsFilename, std::string psFilename, std::string vsUpdateFilename, std::string gsUpdateFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename.c_str(), NULL, NULL, "SnowVS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS || (1 << 0), 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	result = D3DX11CompileFromFile(gsFilename.c_str(), NULL, NULL, "SnowGS", "gs_5_0", D3D10_SHADER_ENABLE_STRICTNESS || (1 << 0), 0, NULL, &geometryShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	result = D3DX11CompileFromFile(psFilename.c_str(), NULL, NULL, "SnowPS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS || (1 << 0), 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	// Compile the vertex shader code.
	result = D3DX11CompileFromFile(vsUpdateFilename.c_str(), NULL, NULL, "SnowUpdateVS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS || (1 << 0), 0, NULL, &vertexShaderUpdateBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	// Compile the geometry shader code.
	result = D3DX11CompileFromFile(gsUpdateFilename.c_str(), NULL, NULL, "SnowUpdateGS", "gs_5_0", D3D10_SHADER_ENABLE_STRICTNESS || (1 << 0), 0, NULL, &geometryShaderUpdateBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	*/
	...
	if (FAILED(result))
	{
		logger->GetInstance().WriteLine("Failed to create polygon layout in Snow Shader class.");
		return false;
	}
	...
	if (FAILED(result))
	{
		logger->GetInstance().WriteLine("Failed to create the sampler state in SnowShader.cpp");
		return false;
	}

	if (!SetupMatrixBuffer(device))
	{
		logger->GetInstance().WriteLine("Failed to set up the matrix buffer in Snow shader class.");
		return false;
	}
	...
	if (FAILED(result))
	{
		logger->GetInstance().WriteLine("Failed to create frame buffer from the description provided in Snow Shader class.");
		return false;
	}
...
void CSnowShader::ShutdownShader()
{
...
void CSnowShader::OutputShaderErrorMessage(ID3D10Blob *errorMessage, HWND hwnd, std::string shaderFilename)
{
	std::string errMsg;
	char* compileErrors;
...
bool CSnowShader::SetShaderParameters(ID3D11DeviceContext* deviceContext)
{
	HRESULT result;
	...
	{
		logger->GetInstance().WriteLine("Failed to set the matrix buffer in the vertex shader in snow shader class.");
		return false;
	}
	if (!SetMatrixBuffer(deviceContext, bufferNumber, ShaderType::Geometry))
	{
		logger->GetInstance().WriteLine("Failed to set the matrix buffer in the geometry shader in snow shader class.");
		return false;
	}
...
void CSnowShader::RenderShader(ID3D11DeviceContext * deviceContext)
{
...
bool CSnowShader::UpdateParticles(ID3D11DeviceContext * deviceContext)
{
	// Set the vertex input layout.
	deviceContext->IASetInputLayout(mpLayout);
	deviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_POINTLIST);
...
void CSnowShader::SetRainTexture(ID3D11ShaderResourceView * resource)
{
	mpDiffuseTexture = resource;
}

void CSnowShader::SetRandomTexture(ID3D11ShaderResourceView * resource)
{
	mpRandomTexture = resource;
}

void CSnowShader::SetCameraWorldPosition(D3DXVECTOR3 pos)
{
	mCameraWorldPosition = pos;
}

void CSnowShader::SetEmitterWorldPosition(D3DXVECTOR3 pos)
{
	mEmitWorldPosition = pos;
}

void CSnowShader::SetEmitterWorldDirection(D3DXVECTOR3 dir)
{
	mEmitWorldDirection = dir;
}

void CSnowShader::SetGameTime(float time)
{
	mGameTime = time;
}

void CSnowShader::SetFrameTime(float updateTime)
{
	mFrameTime = updateTime;
}

void CSnowShader::SetGravityAcceleration(float gravityAcceleration)
{
	mGravityAcceleration = gravityAcceleration;
}

void CSnowShader::SetWindX(float windX)
{
	mWindX = windX;
}

void CSnowShader::SetWindZ(float windZ)
{
	mWindZ = windZ;
}

void CSnowShader::SetFirstRun(bool firstRun)
{
	mFirstRun = firstRun;
}